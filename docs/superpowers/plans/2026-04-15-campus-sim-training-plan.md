...
## Resume card (last update: 2026-04-16)

**Branch:** `feat/campus-sim` (11 commits ahead of `main`, not pushed)

**Tasks done: 0 ✅ 1 ✅ 2 ✅ 3 ✅ 4 ✅ 5 ✅**
**Next up: Task 6 — Action adapter (steer/throttle → cmd_vel)**

### Commits (chronological, oldest → newest)

| Task | SHAs | Title |
|------|------|-------|
| 0    | `2c6d95a`, `d845eac`, `b8642c5` | scaffold: three-surface project layout + couchmo_expert package |
| 1    | `f25fced`, `af5fbf1` | sim: cross-platform bringup (.sh + .ps1) using noVNC |
| 2    | `ad97221` | sim: dual armrest cameras (Brio 100, 32 in) in URDF |
| 3    | `5fcfae2`, `6523b77` | tools: KML/KMZ → ENU waypoint CSV |
| 4    | `a11b908` | sim: hybrid campus world generator + synthetic .world |
| 5    | `b049e3c`, `8c268ee` | sim: pure-pursuit waypoint expert @10 Hz |

### Host-side verification state

- ✅ All 22 unit tests pass via direct execution on a Windows host without pytest, ROS, or Docker installed:
  - `python training/tests/test_pure_pursuit.py` → 8/8
  - `python training/tests/test_kml_to_waypoints.py` → 7/7
  - `python training/tests/test_world_generation.py` → 7/7
  - `python training/tests/test_preprocess.py` → 4/4 (needs `PYTHONPATH=.`)
  - `python training/tests/test_dataset_format.py` → round-trip verified
- ✅ `python simulation/scripts/bringup_check.py` → Docker reachable, port 6081 free, ≥8 GB RAM
- ✅ `simulation/run.ps1` parses (`[scriptblock]::Create(...)`) and `docker-compose.yml` resolves to services `[sim, preview, sim_headless]`
- ✅ All generated XML artifacts (URDF/xacro, .world) well-formed; regeneration is byte-deterministic with `--seed 0`

### What has NOT been verified (requires Docker + ROS — deferred to user)

- `.\run.ps1 sim` actually standing up Gazebo + noVNC in a browser
- `ros2 topic list | grep image_raw` showing `/left_cam/image_raw` + `/right_cam/image_raw`
- `colcon build --packages-select couchmo_expert` completing
- The waypoint expert driving the couch around the synthetic campus world at 10 Hz

### How to resume on a different machine

1. `git clone <this repo>` and `git checkout feat/campus-sim`
2. (Optional) install pytest to run the suite: `cd training && pip install -r requirements.txt -r requirements-dev.txt && python -m pytest -q`
3. Open this plan file; the `[x]` vs `[ ]` checkbox state IS the source of truth for what's done
4. Start a fresh context window. Tell the agent: **"Resume execution of `docs/superpowers/plans/2026-04-15-campus-sim-training-plan.md` using superpowers:subagent-driven-development starting at Task 6. Tasks 0-5 are already complete on branch feat/campus-sim."**
5. The agent should read this resume card, verify the branch + last commit match `8c268ee`, then dispatch the Task 6 implementer.

### Known plan follow-ups (non-blocking, deferred from earlier reviews)

- Task 2: promote hardcoded sensor constants in `couchmo.urdf.xacro` (cube size, mass, inertia, clip near/far, noise stddev) to `<xacro:property>` for the domain-randomization task that will land later.
- Task 4: de-duplicate metadata comment that's emitted in two places with different field orders in the generated `.world` file.
- Task 5: two cosmetic annotation/docstring nits flagged but deferred by the code reviewer (local `info: dict[str, Any]` annotation and a drifted docstring inside `compute()`).
...
- [x] **Step 2: `.gitattributes` + `.gitignore`**
  - `.gitattributes`:
    ```
    * text=auto
    *.sh text eol=lf
...
- [x] **Step 3: `couchmo_expert` ROS 2 package skeleton**
...
- [x] **Step 5: `shared/dataset_format.py`**
...
- [x] **Step 6: `training/` Python project**
...
- [x] **Step 7: `runtime/` Python project**
  - `requirements.txt`:
    ```
    onnxruntime>=1.17
    opencv-python>=4.8
...
- [x] **Step 1: `bringup_check.py` (cross-platform Python)**
  - Stdlib only. Checks:
...
- [x] **Step 2: Rewrite `run.sh` minimally**
...
- [x] **Step 6: Verify (host-side, where possible)**
...
- [x] **Step 1: Define camera links + joints**
...
- [x] **Step 1: Real failing tests for KML parsing**
...
- [x] **Step 3: Generate the synthetic-campus world from Task 0's fixture**
  ```bash
...
- [x] **Step 1: Real failing tests** (pure logic, no ROS)
...
- [x] **Step 2: Implement expert as plain Python class** in `waypoint_expert_node.py` plus a thin `rclpy.Node` wrapper at the bottom.
...
- [x] **Step 3: YAML config** with `corridor_width_m: 3.0`, `lookahead_min_m`, `lookahead_max_m`, `max_throttle`, `curvature_throttle_gain`.

- [x] **Step 4: Launch file** — loads waypoints CSV, starts the node.

- [x] **Step 5: Register entry point in `setup.py`**.

- [x] **Step 6: Run unit tests (expect pass)**
  ```bash
